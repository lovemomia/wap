    ...
    public ModelAndView skuplace(@RequestParam long id,
        ...
        switch (status) {
            case SKU_PLACE_STATUS_CURRENT_MONTH:
                resp = get("/course/sku/month" + (pid > 0 ? "/bookable" : "/notended") + "?id=" + id + "&month=" + month);
                break;
            case SKU_PLACE_STATUS_NEXT_MONTH:
                resp = get("/course/sku/month" + (pid > 0 ? "/bookable" : "/notended") + "?id=" + id + "&month=" + nextMonth);
                break;
            default:
                resp = get("/course/sku/week" + (pid > 0 ? "/bookable" : "/notended") + "?id=" + id);
        }
