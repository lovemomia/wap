...
public class CourseController extends AbstractController {
    ...
    public ModelAndView course(HttpServletRequest request, @RequestParam long id, @RequestParam(value = "sid", defaultValue = "0") long skuId) {
        String utoken = getUtoken(request);
        return new ModelAndView("course/course", "course", get("/v3/course?id=" + id + (StringUtils.isBlank(utoken) ? "" : "&utoken=" + utoken) + (skuId > 0 ? "&sid=" + skuId : "")));
    }

    @RequestMapping(value = "/trial", method = RequestMethod.GET)
    public ModelAndView trial(HttpServletRequest request, @RequestParam long id, @RequestParam(value = "sid", defaultValue = "0") long skuId) {
        String utoken = getUtoken(request);
        JSONObject courseJson = (JSONObject) get("/v3/course?trial=1&id=" + id + (StringUtils.isBlank(utoken) ? "" : "&utoken=" + utoken) + (skuId > 0 ? "&sid=" + skuId : ""));
    ...
    public ModelAndView buyable(HttpServletRequest request, @RequestParam long id, @RequestParam(value = "sid", defaultValue = "0") long skuId) {
        String utoken = getUtoken(request);
        JSONObject courseJson = (JSONObject) get("/v3/course?recommend=1&id=" + id + (StringUtils.isBlank(utoken) ? "" : "&utoken=" + utoken) + (skuId > 0 ? "&sid=" + skuId : ""));
